...
export async function createRole(roleData) {
  ...
  const [existing] = await pool.query(
    "SELECT id FROM sys_role WHERE code = ? AND del = 0",
    [code],
  );
...
export async function updateRole(id, roleData) {
  ...
  if (updates.length > 0) {
    values.push(id);
    await pool.query(
      `UPDATE sys_role SET ${updates.join(", ")} WHERE id = ?`,
      values,
    );
  }
...
export async function deleteRole(id) {
  const [users] = await pool.query(
    "SELECT id FROM sys_user WHERE role_id = ? AND del = 0",
    [id],
  );
  if (users.length > 0) {
  ...
  await pool.query("DELETE FROM sys_role_permission WHERE role_id = ?", [id]);
  const [result] = await pool.query(
    "UPDATE sys_role SET del = 1 WHERE id = ?",
    [id],
  );
